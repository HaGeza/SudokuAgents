...
class GameTree:
    ...
    def _is_taboo_state(self) -> bool:
        ...
        return np.any(
            (np.sum(self.available, axis=2) +
             (self.board != SudokuBoard.empty)) == 0
        )
    ...
    def _apply_move(self, move: Move, maximizer: bool, last_depth: bool) -> GameTree:
        ...
        reward = (
            ...
            - last_depth * self.PENALTY[row_cnt ==
                                        1][col_cnt == 1][box_cnt == 1]
        )
    ...
    def _get_possible_moves(self) -> [Move]:
        ...
        values_available = values_available[available_inds[:,
                                                           0], available_inds[:, 1]]
        ...
        length_to_keep = max(
            np.count_nonzero(values_available <=
                             self.VALUE_FILTER["available_le"]),
    ...
    def from_game_state(game_state: GameState) -> GameTree:
        ...
        for taboo_move in game_state.taboo_moves:
            gt.available[taboo_move.i, taboo_move.j,
                         taboo_move.value - 1] = False
