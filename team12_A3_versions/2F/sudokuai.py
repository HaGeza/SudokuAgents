...
class GameTree:
    ...
    def _get_all_missing_counts(self) -> (np.array, np.array, np.array):
        """
        Get the number of missing values in each row, column and block. 
        """
        ...
        row_cnt = np.sum(self.board == SudokuBoard.empty, axis=1)
        col_cnt = np.sum(self.board == SudokuBoard.empty, axis=0)
        box_cnt = np.array([[np.sum(self.board[ii:ii+m,jj:jj+n] == SudokuBoard.empty) \
                            for jj in range(0, N, n)] for ii in range(0, N, m)])

        return row_cnt, col_cnt, box_cnt


    def _get_relevant_moves(self) -> [Move]:
        """
        ...
        # Count the number of missing values in each region
        row_cnt, col_cnt, box_cnt = self._get_all_missing_counts()
        priorities = row_cnt[available_inds[:, 0]] + \
                     col_cnt[available_inds[:, 1]] + \
                     box_cnt[available_inds[:, 0] // m, \
                             available_inds[:, 1] // n]
        ordering = np.argsort(priorities)

        for i in ordering:
            inds = available_inds[i]
            if TabooMove(*inds) in self.taboo_moves:
                continue

            if row_cnt[inds[0]] > 2 and col_cnt[inds[1]] > 2 and \
                    box_cnt[inds[0] // m, inds[1] // n] > 2:
                return [Move(*inds)]
            else:
                break

        return [Move(*inds) for inds in available_inds[ordering] \
                if TabooMove(*inds) not in self.taboo_moves]
    ...
    def minimax(self, depth: int, maximizer: bool, alpha: float, beta: float) -> (float, Move, int):
        ...
        all_moves = self._get_relevant_moves()
        if len(all_moves) == 0:
            return (self._evaluate(), None, 0)
